...
class SPModel(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        self.qc_att = BiAttention(config.hidden*8, 1-config.keep_prob)
        self.linear_1 = nn.Sequential(
                nn.Linear(config.hidden*8, config.hidden*8),
                nn.ReLU()
            )
        self.ques_att = BiAttention(config.hidden*2, 1-config.keep_prob)
        self.context_att = BiAttention(config.hidden*2, 1-config.keep_prob)
        self.conv = nn.Conv1d(in_channels=config.hidden*32, out_channels=config.hidden*4, kernel_size=5, padding=2, bias=False)
        pytorch_total_params = sum(p.numel() for p in self.conv.parameters() if p.requires_grad)
        # print ("show total params", pytorch_total_params)
        self.linear_before_res1 =  nn.Sequential(
            nn.Linear(config.hidden*8, config.hidden*2),
            nn.ReLU()
        )
        self.linear_before_res2 = nn.Sequential(
            nn.Linear(config.hidden*8, config.hidden*2),
            nn.ReLU()
        )
        ...
        self.rnn_sp = EncoderRNN(config.hidden*4, config.hidden, 1, False, True, 1-config.keep_prob, False)
        self.linear_sp = nn.Linear(config.hidden*2, 1)

        self.rnn_start = EncoderRNN(config.hidden*6, config.hidden, 1, False, True, 1-config.keep_prob, False)
        self.linear_start = nn.Linear(config.hidden*2, 1)

        self.rnn_end = EncoderRNN(config.hidden*6, config.hidden, 1, False, True, 1-config.keep_prob, False)
        self.linear_end = nn.Linear(config.hidden*2, 1)

        self.rnn_type = EncoderRNN(config.hidden*6, config.hidden, 1, False, True, 1-config.keep_prob, False)
    ...
    def forward(self, context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, start_mapping, end_mapping, all_mapping, return_yp=False):
        ...
        ques_output_with_att = self.ques_att(ques_output, ques_output, ques_mask)
        context_output_with_att = self.context_att(context_output, context_output, context_mask)
        qo_w_att = self.linear_before_res1(ques_output_with_att)
        co_w_att = self.linear_before_res2(context_output_with_att)
        # co = co_w_att + context_output
        # qo = qo_w_att + ques_output
        output = self.qc_att(context_output_with_att, ques_output_with_att, ques_mask)
        output = output.permute(0, 2, 1)
        output = self.conv(output)
        # print ("show op", output.size())
        output = output.permute(0, 2, 1)
        # print ("show op222", output.size())
        
        # output = self.linear_1(output)
        # print ("show op3", output.size())
