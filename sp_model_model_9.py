...
class SPModel(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        self.linear_1 = nn.Sequential(
                nn.Linear(config.hidden*32, config.hidden*8),
                nn.ReLU()
            )
        ...
        self.rnn_sp = EncoderRNN(config.hidden*8, config.hidden, 1, False, True, 1-config.keep_prob, False)
        self.linear_sp = nn.Linear(config.hidden*2, 1)

        self.rnn_start = EncoderRNN(config.hidden*10, config.hidden, 1, False, True, 1-config.keep_prob, False)
