...
class SPModel(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        self.linear_1 = nn.Sequential(
                nn.Linear(config.hidden*32, config.hidden*12),
                nn.ReLU()
            )
        ...
        self.linear_before_res1 =  nn.Sequential(
            nn.Linear(config.hidden*12, config.hidden*2),
            nn.ReLU()
        )
        ...
        self.rnn_sp = EncoderRNN(config.hidden*12, config.hidden, 1, False, True, 1-config.keep_prob, False)
        self.linear_sp = nn.Linear(config.hidden*2, 1)

        self.rnn_start = EncoderRNN(config.hidden*14, config.hidden, 1, False, True, 1-config.keep_prob, False)
        self.linear_start = nn.Linear(config.hidden*2, 1)

        self.rnn_end = EncoderRNN(config.hidden*14, config.hidden, 1, False, True, 1-config.keep_prob, False)
        self.linear_end = nn.Linear(config.hidden*2, 1)

        self.rnn_type = EncoderRNN(config.hidden*14, config.hidden, 1, False, True, 1-config.keep_prob, False)
...
class BiAttention(nn.Module):
    def __init__(self, input_size, dropout):
        super().__init__()
        self.dropout = LockedDropout(dropout)
        self.input_linear = nn.Linear(input_size, 1, bias=True)
        self.memory_linear = nn.Linear(input_size, 1, bias=True)
